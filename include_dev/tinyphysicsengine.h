...
TPE_Vec3 TPE_envGround(TPE_Vec3 point, TPE_Unit height)
{
  ...
  /* In float mode we just reinterpret the bits of a float as uint32_t.
     This is safe on any platform that uses IEEE-754 single-precision floats
     (the overwhelming majority of modern compilers). */
  static inline uint32_t _TPE_floatBits(float f)
  {
    uint32_t bits;
  ...
  /* Very small mixing function – enough for our deterministic hash needs. */
  static inline uint32_t _TPE_hash(uint32_t n)
  {
    n ^= n >> 17;
    n *= 0x9E3779B9u;          // golden-ratio based constant
    n ^= n >> 15;
    n *= 0x85EBCA6Bu;
    n ^= n >> 13;
    return n;
  ...
  static inline uint32_t TPE_jointHash(const TPE_Joint *joint)
  {
    uint32_t r = 0;

    r = _TPE_hash(r ^ _TPE_floatBits(joint->position.x));
    r = _TPE_hash(r ^ _TPE_floatBits(joint->position.y));
    r = _TPE_hash(r ^ _TPE_floatBits(joint->position.z));

    /* velocity is stored as three TPE_UnitReduced (int16_t) in int mode,
       as three floats in float mode.  Pack them into a single 32-bit word. */
    uint32_t vx = _TPE_floatBits(joint->velocity[0]);
    uint32_t vy = _TPE_floatBits(joint->velocity[1]);
    uint32_t vz = _TPE_floatBits(joint->velocity[2]);

    uint32_t velPack = (vx & 0xFFFFu) | ((vy & 0xFFFFu) << 16);
    uint32_t velSize = (vz & 0xFFFFu) | ((uint32_t)joint->sizeDivided << 16);

    r = _TPE_hash(r ^ velPack);
    r = _TPE_hash(r ^ velSize);
...
#endif /* TPE_USE_FLOAT */
/* --------------------------------------------------------------------- */
/*  The rest of the hash helpers – identical for both modes               */
/* --------------------------------------------------------------------- */
